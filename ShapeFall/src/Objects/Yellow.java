...
public class Yellow extends GameObject{
	...
	public Yellow(float x, float y, float width, float height, float fallSpeed, Color color, Handler handler, ObjectId id) {
	...
	public void render(Graphics g) {
		...
		if(shape == 1){
			...
			g.fillRect((int)x, (int)y, (int)width, (int)height);
			System.out.println("YELLOW ERROR 1");
		}
		
		if(handler.numbersEnabled()){
			g.drawImage(tex.mini[2], (int)x+2, (int)y+2, (int)width-4, (int)height-4, null);
		}
		...
		if(shape == 1){
			...
			g2d.draw(getBounds());
			System.out.println("YELLOW ERROR 2");
		}
	}
